...
│   ├── 99-D-视频上传调试记录.md
│   ├── 102-操作审计日志.md
│   ├── 103-小程序iOS端chooseVideo选择视频转圈兼容性修复.md
│   ├── 104-微信内容安全API集成.md
│   ├── 105-骨架视频逐帧生成条件分支.md
│   ├── 106-帧引用传递优化.md
│   ├── 107-小程序埋点上报优化.md
...
│   ├── 114-Admin文件预览与下载.md
│   ├── 116-文件类型探测与修复.md
│   ├── 117-原视频信息表与衍生文件登记.md
│   ├── 118-分析流水线重构与视频信息表.md
│   ├── 119-电子教练后台任务队列与管线模式.md
│   ├── 120-Pipeline耗时日志与前端旧模式清理.md
│   ├── 121-写表步骤批量插入优化.md
│   ├── 122-管线文件管理优化.md
│   ├── 123-综合分析骨骼视频完整绘制与骨架帧清理.md
│   ├── 124-Admin分析报告时间修复与筛选功能.md
│   ├── 125-Admin文件管理简化与查询优化.md
...
| 115：pytest 测试提速方案 | v1.0.0 | 方案 | `plans/115-pytest测试提速方案.md` | 内存库 StaticPool + session 级 client + xdist 并行 + testmon + fast 分层门禁 + 全量 CI | ✅ 已完成 |
...
| 99-D：视频上传调试记录 | v1.0.0 | 方案 | `plans/99-D-视频上传调试记录.md` | 电子教练视频上传调试：设备端「无法解析视频时长」根因排查与修复 | ✅ 已完成 |
...
| 102：操作审计日志 | v1.4.0 | 方案 | `plans/102-操作审计日志.md` | 全量写操作审计（Admin+小程序端41个端点含登录）+ 独立审计库 + 中间件+装饰器方案 + Admin前端页面 + 13测试 + created_at UTC Z 后缀 | ✅ 已完成 |
| 103：小程序 iOS 端 chooseVideo 选择视频转圈兼容性修复 | v1.0.0 | 方案 | `plans/103-小程序iOS端chooseVideo选择视频转圈兼容性修复.md` | `uni.chooseVideo` → `uni.chooseMedia` 迁移 + iOS 端选择超时检测 + 增强错误处理 | 🚧 进行中 |
| 108：日记装备关联选择与手写 | v1.1.0 | 方案 | `plans/108-日记装备关联选择与手写.md` | 支持从已有装备列表选择 + 手动输入；修复装备选择区 flex 布局挤压 + 选中/手输名称回显 | 🏁 已完成 |
| 117-原视频信息表与衍生文件登记 | v1.2.0 | 方案 | `plans/117-原视频信息表与衍生文件登记.md` | 原视频保留 + 原视频信息表设计（已并入 118） | ⏳ 已归档 |
| 118-分析流水线重构与视频信息表 | v1.0.0 | 方案 | `plans/118-分析流水线重构与视频信息表.md` | 提前建 analysis_id 的分步更新流水线 + analysis_video_info 表 + 播放短片入文件管理 + 分类对齐边界 | 📋 待执行 |
| 119-电子教练后台任务队列与管线模式 | v1.2.0 | 方案 | `plans/119-电子教练后台任务队列与管线模式.md` | 电子教练分析流程改造为后台任务队列+管线模式+状态管理：前端仅上传视频，后端异步执行完整分析管线，支持混合模式状态订阅（轮询+SSE） | 🚧 进行中 |
| 121：写表步骤批量插入优化 | v1.5.0 | 方案 | `plans/121-写表步骤批量插入优化.md` | Pipeline 写表步骤批量插入优化（骨架帧 N+1 查询问题） | ✅ 已完成 |
| 122：管线文件管理优化 | v1.2.0 | 方案 | `plans/122-管线文件管理优化.md` | 裁剪视频纳入文件管理 + 采样帧清理 + 移除 skeleton_thumb 冗余字段 | 🚧 进行中 |
| 123：综合分析骨骼视频完整绘制与骨架帧清理 | v1.2.0 | 方案 | `plans/123-综合分析骨骼视频完整绘制与骨架帧清理.md` | full 模式骨骼视频逐帧绘制 + 骨架帧清理 + 缩略图独立 + 删除兜底 + Admin弹窗视频播放器 | 📋 待执行 |
| 124：Admin 分析报告时间修复与筛选功能 | v1.0.0 | 方案 | `plans/124-Admin分析报告时间修复与筛选功能.md` | 修复创建时间显示 1970/1/22 + 排序混乱 + 添加筛选条件与查询按钮 | 📋 待执行 |
| 125：Admin 文件管理简化与查询优化 | v1.1.0 | 方案 | `plans/125-Admin文件管理简化与查询优化.md` | 删减冗余端点/Schema + 列表 N+1 修复 + bulk_classify 统一 | 🏁 已完成 |
...
| 115-pytest测试提速方案 | 测试与工程优化 | ✅ 已完成 | 2026-08-27 | 2026-08-27 |
...
| 99-D-视频上传调试记录 | 小程序前端 / 后台排障 | ✅ 已完成 | 2026-08-20 | 修复设备端上传视频时长解析失败（Android 分片上传 Content-Length 问题） |
| 101-后端日志细化与异常静默处理修复 | 后台 | ✅ 已完成 | 2026-08-21 | 11 文件修改 + ruff check/format 通过 + 126 tests passed |
| 102-操作审计日志 | 后台/审计/Admin前端 | ✅ 已完成 | 2026-08-21 | 后端 13 测试通过 + ruff check/format 通过 + admin build 通过；41 端点全量装饰器含登录 + 独立审计库 + 中间件+装饰器架构 + Admin 查询端点 + 前端审计日志页 + created_at UTC Z 后缀 |
| 103-小程序iOS端chooseVideo选择视频转圈兼容性修复 | 小程序前端 | 🚧 进行中 | 2026-08-22 | - |
| 104-微信内容安全API集成 | 小程序前端/后台 | 🚧 进行中 | 2026-08-22 | - |
| 105-骨架视频逐帧生成条件分支 | 小程序前端/后台 | ✅ 已完成 | 2026-08-22 | - |
| 106-帧引用传递优化 | 小程序前端 | ✅ 已完成 | 2026-08-22 | - |
| 107-小程序埋点上报优化 | 小程序前端/Admin | ✅ 已完成 | 2026-08-23 | type-check + build 通过 |
| 108-日记装备关联选择与手写 | 小程序前端 | 🏁 已完成 | 2026-08-23 | 2026-09-02 | 修复装备选择区 flex 布局挤压 + 选中回显；type-check + build:mp-weixin 通过 |
| 109-文件管理系统 | 后台/Admin前端/小程序 | 🚧 进行中（v1.4.0） | 2026-08-24 | 独立 File 模型 + 秒传 + 软删除联动 + Admin 文件管理页 + 约束修复（md5→original_name） |
| 110-文件扫描功能 | 后台/Admin前端 | 🚧 进行中 | 2026-08-24 | 扫描 uploads 目录，将未注册文件纳入 File 表管理 |
| 111-文件使用标记 | 后台/Admin前端 | 🏁 已完成 | 2026-08-25 | 基于业务表引用核查，实时计算 usage_status / usage_reason，Admin 列表徽章 + 统计 + 扫描弹窗展示 |
| 117-原视频信息表与衍生文件登记 | 后台/文件管理/电子教练 | ⏳ 已归档 | 2026-08-28 | 原视频保留 + 原视频信息表设计（已并入 118） |
| 118-分析流水线重构与视频信息表 | 后台/文件管理/电子教练/小程序 | 📋 待执行 | 2026-08-28 | 提前建 analysis_id 的分步更新流水线 + analysis_video_info 表 + 播放短片入文件管理 + 分类对齐边界 |
| 119-电子教练后台任务队列与管线模式 | 后台/电子教练/小程序 | 🚧 进行中 | 2026-08-30 | 电子教练分析流程改造为后台任务队列+管线模式+状态管理 |
| 121-写表步骤批量插入优化 | 后台/电子教练 | ✅ 已完成 | 2026-08-31 | Pipeline 写表步骤批量插入优化（预计算 MD5/size + 批量文件登记 + 合并 commit，56.5s→<1s） |
| 122-管线文件管理优化 | 后台/电子教练 | 🚧 进行中 | 2026-08-31 | 裁剪视频纳入文件管理 + 采样帧清理 + 移除 skeleton_thumb 冗余字段 |
| 123-综合分析骨骼视频完整绘制与骨架帧清理 | 后台/电子教练 | 📋 待执行 | - | full 模式骨骼视频逐帧绘制 + 骨架帧清理 + 缩略图独立 + 删除兜底 |
| 124-Admin分析报告时间修复与筛选功能 | 后台管理端优化 | 📋 待执行 | - | 修复创建时间显示 1970/1/22 + 排序混乱 + 添加筛选条件与查询按钮 |
| 125-Admin文件管理简化与查询优化 | 后台管理端优化 | 🏁 已完成 | 2026-09-02 | 删减冗余端点/Schema + 列表 N+1 修复 + bulk_classify 统一 |
