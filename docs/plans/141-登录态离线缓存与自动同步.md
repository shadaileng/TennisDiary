...
> | 2026-09-11 | v1.1.0 | 电子教练调整：仅缓存分析列表（不缓存详情，详情页纯在线）；新增 useAnalysisStore；移除详情 LRU 缓存与 analysisDetailKey/analysisIndexKey |
>
...
为**已登录用户**的日记/装备/体重/分析记录增加离线可用能力：**本地缓存作为列表渲染的唯一数据源**（分析记录仅缓存列表，详情页始终在线拉取），页面先渲染缓存合并视图再按需刷新；网络不可用或远程服务异常时仍可浏览缓存并新建记录（入本地待同步仓库），网络恢复后自动静默同步；游客态现有本地降级机制（Step 129）保持零改动。
...
- **离线浏览**：请求失败只标记「离线/缓存」状态并显示横幅，**绝不因失败清空或覆盖当前缓存视图**；首次且无缓存时才展示"网络不可用"空态。日记/装备/体重可离线浏览列表与（缓存命中的）详情；**分析记录仅缓存历史列表**，报告详情（含六维点评/pose/视频）体积大、媒体离线也看不了，故详情页始终在线拉取——离线进入详情仅提示「当前离线，报告详情需联网后查看」。视频/封面等媒体离线统一本地占位、不发起请求、不黑屏破图。
...
2. **新增账号隔离仓库（零 store 依赖，防循环）**：
   ...
   - `cloudCache.ts`：账号隔离云端快照，键 `td_cache_{userId}_diaries/gears/weights/analyses`（列表 + updatedAt meta）；**分析仅缓存列表**（写入时已裁剪为轻量字段：剥离 report/pose/highlights/video_url，丢弃 dataURL 封面，避免单键超 900KB），不缓存详情。
3. **store 数据源统一为缓存合并视图**（diary/gear/weight 三 store 同构改造；分析列表新增 `useAnalysisStore` 同构改造，仅缓存列表、不接 offlineRepo）：
   ...
   - `analysis.ts`（**新增 `useAnalysisStore`，列表仅缓存、无离线新建，与 diary/gear/weight 的差异点**）：state = analyses/loading/offline/fromCache；`hydrate()` 从 cloudCache 仅读分析列表（轻量字段）；`fetchList()` 登录+网络可用才 GET，成功写 cloudCache 并替换 state，失败(status=-1) 置 offline 标记、不清空；`removeAnalysis(id)` 删除后同步云端缓存视图。分析记录依赖服务端视频/AI/姿态流水线、无法离线新建，故不接 offlineRepo/pendingRepo；**报告详情不在此 store，report.vue 改为纯在线拉取**。
   - 判定离线：仅 `ApiError.status === -1`（网络层失败）视为离线降级；4xx 业务错误不降级（避免掩盖业务失败），401 仍走既有 clearAuth/登出。
4. **分析记录接入（列表缓存、详情在线）**：coach.vue 列表渲染源 = `useAnalysisStore.analyses`（缓存优先 + 后台刷新，与日记/装备数据驱动一致）；report.vue 详情改为**纯在线拉取**（不再缓存单条详情），离线进入仅提示「当前离线，报告详情需联网后查看」。`<image>/<video>` 媒体统一走 `resolveMediaSrc(url, networkOnline)`（utils/media.ts）——远程 URL 离线时返回本地占位素材 `static/offline-media.png`，dataURL/本地路径原样；`<image>` 保留 `@error` 兜底换占位（防 URL 失效/鉴权 401），离线 `<video>` 不加载并显示「网络不可用」遮罩。
...
miniapp/src/services/cloudCache.ts           # [NEW] 账号隔离云端快照（diaries/gears/weights/analyses 列表 + updatedAt；分析仅缓存列表，详情不缓存）
...
miniapp/src/stores/analysis.ts               # [NEW] useAnalysisStore：分析列表仅缓存（hydrate/fetchList/removeAnalysis），无离线新建
...
miniapp/src/pages/coach/coach.vue            # [MODIFY] 列表绑定 useAnalysisStore（缓存优先 + 后台刷新；onShow/下拉刷新/分析启动事件触发）；加载中/空态三态与日记一致
miniapp/src/pages/coach/report.vue           # [MODIFY] 详情改为纯在线拉取（不缓存），离线提示需联网；删除后调用 removeAnalysis 同步列表缓存
...
- 手动场景：断网浏览（缓存命中 / 首次无缓存空态）、断网新建三实体本地可见带待同步标记、恢复后自动同步无重复、换号/登出隔离、游客路径回归、stats 离线聚合兜底、装备封面/分析缩略图/报告视频离线占位不破图；**分析历史列表离线可回看，报告详情离线进入提示需联网（不缓存详情）**
