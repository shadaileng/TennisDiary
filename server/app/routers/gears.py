...
def list_gears(
    ...
    return [GearResponse.model_validate(g) for g in gears]


@router.post("", response_model=GearResponse, status_code=status.HTTP_200_OK)
def create_gear(
    body: GearCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
def get_gear(
    ...
    """装备详情"""
    return GearResponse.model_validate(_get_owned_gear(db, gear_id, current_user))
