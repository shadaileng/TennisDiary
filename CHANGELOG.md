...
- 修复时间轴裁剪引导缺失导致整片超限直传（99 时间轴）：客户端 `startAnalysis` 预检查漏判「视频时长未加载（`videoDuration=0`）」场景——此时 `0 > modeLimit` 恒为假，整片 120s+ 视频会被放行上传，撞上后端「单次挥拍视频最长 15 秒，当前 120.2 秒」这类费解报错。现时长未加载或整片超限且未裁剪时均在本地拦截并引导到时间轴；后端超限兜底消息补充「请在时间轴选择片段（裁剪后总长 ≤ N 秒）」提示。
